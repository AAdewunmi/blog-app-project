...
class PostResponseTest {
    ...
    void shouldGenerateToString() {
        String toString = postResponse.toString();

        assertAll("ToString verification",
                () -> assertTrue(toString.contains("content="), "ToString should contain content field"),
                () -> assertTrue(toString.contains("pageNumber=" + PAGE_NUMBER), "ToString should contain page number"),
                () -> assertTrue(toString.contains("pageSize=" + PAGE_SIZE), "ToString should contain page size"),
                () -> assertTrue(toString.contains("totalElements=" + TOTAL_ELEMENTS), "ToString should contain total elements"),
                () -> assertTrue(toString.contains("totalPages=" + TOTAL_PAGES), "ToString should contain total pages"),
                () -> assertTrue(toString.contains("lastPage=" + LAST_PAGE), "ToString should contain last page flag")
        );
    }

    /**
     * Tests the equals and hashCode methods generated by Lombok.
     * Verifies that objects with the same content are equal and have the same hash code,
     * while objects with different content are not equal and have different hash codes.
     */
    @Test
    @DisplayName("Should properly implement equals and hashCode")
    void shouldImplementEqualsAndHashCode() {
        PostResponse sameResponse = new PostResponse(
                content,
                PAGE_NUMBER,
                PAGE_SIZE,
                TOTAL_ELEMENTS,
        ...
        PostResponse differentResponse = new PostResponse(
                new ArrayList<>(),
                1,
                5,
                10,
        ...
        assertAll("Equals and HashCode",
                () -> assertEquals(postResponse, sameResponse, "Same content should be equal"),
                () -> assertEquals(postResponse.hashCode(), sameResponse.hashCode(), "Same content should have same hash"),
                () -> assertNotEquals(postResponse, differentResponse, "Different content should not be equal"),
                () -> assertNotEquals(postResponse.hashCode(), differentResponse.hashCode(), "Different content should have different hash")
        );
    }
}